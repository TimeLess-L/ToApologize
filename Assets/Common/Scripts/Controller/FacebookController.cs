...
public class FacebookController : MonoBehaviour
{
    ...
    public void LoginFacebookForPublish()
    {
        //if (!FB.IsInitialized) return;
    ...
    {
        return GetScreenshotTexture().EncodeToPNG();
    }
    ...
    public void PostScreenshot(byte[] data, string message)
    {
        var wwwForm = new WWWForm();
        wwwForm.AddBinaryData("image", data, "screenshot.png");
        wwwForm.AddField("message", message);

        //FB.API("me/photos", HttpMethod.POST, HandleResultPostScreenshot, wwwForm);
    }

    //private void HandleResultPostScreenshot(IResult result)
    //{
    //    if (CheckFBResult(result))
    //    {
    //        Toast.instance.ShowMessage("Posting is successful");
    //    }
    //}
    #endregion
    ...
    private void GetInvitableFriends(JSONArray data)
    {
        ...
        else if (invitableOffset != 0 && to > data.Count + delta)
        {
            invitableOffset = 0;
    ...
    #region Pay
    public void Pay(string productID, Action onPayComplete)
    {
        this.onPayComplete = onPayComplete;
        //FB.Canvas.PayWithProductId(productID, quantity: 1, callback: HandlePayResult);
    }

    //private void HandlePayResult(IResult result)
    //{
    //    if (CheckFBResult(result))
    //        onPayComplete();
    //}
